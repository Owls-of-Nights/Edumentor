...
const tableOfContents = [
  { title: "Introduction to React", link: "#introduction" },
  { title: "Components", link: "#components" },
  { title: "Props", link: "#props" },
  { title: "State", link: "#state" },
  { title: "Component Lifecycle", link: "#lifecycle" },
]
...
export default function ReactCoursePage() {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-8">React Course</h1>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="grid w-full grid-cols-5">
          {Object.keys(content).map((key) => (
            <TabsTrigger key={key} value={key} id={key}>
              {content[key].title}
            </TabsTrigger>
          ))}
        </TabsList>
